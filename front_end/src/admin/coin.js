...
const Coin = () => {
    ...
    const PostDat = (e) =>{
        try{
            setAlert(false)
            e.preventDefault()
            axios.post(`${"https://stawro.xyz"}/coin/new/data`,{title, img, valid, body, stars})
            .then(res =>{
    ...
    const GetStoreCoins = () =>{
        try{
            setTimeout(()=>{
                apiAdmin.get(`${"https://stawro.xyz"}/get/coin/data/2`)
                .then(res=>{
                if(res.data.data){
          ...
          {store_coins.map((data, i) =>{
                const Dele = (e) =>{
                    try{
                        e.preventDefault()
                        setAlert(false)
                        axios.delete(`${"https://stawro.xyz"}/delete/coin/by/${data._id}`)
                        .then(res=>{
                            if(res.data.Status === "OK"){
                                GetStoreCoins();
                            }else{
