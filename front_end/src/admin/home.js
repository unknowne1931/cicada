...
const AdminHome = () => {
    ...
    const GetTotal = () =>{
        try{
            apiAdmin.get(`${"https://stawro.xyz"}/get/aal/tottttal/users`)
            .then(res =>{
                if(res.data.users){
    ...
    const GetTotalWinners = () =>{
        try{
            apiAdmin.get(`${"https://stawro.xyz"}/get/total/users/by/winners/datas/all`)
            .then(res =>{
                if(res.data.users){
    ...
    const GetLive = () =>{
        try{
            setTimeout(()=>{
                apiAdmin.get(`${"https://stawro.xyz"}/admin/get/all/users/data/logined`)
                .then(res =>{
                    if(res.data.users){
    ...
    const PostLineData = (e) =>{
        try{
            e.preventDefault()
            setAlert(false);
            apiAdmin.post(`${"https://stawro.xyz"}/length/and/calcul/ation/of/chart`)
            .then(res =>{
                    ...
                    {win_data.map((user, i)=>{
                        return(
                            <div className='admin-home-main-cnt-01-sub-cnt-01-sunb-cnt-01-sub-cnt-01'>
                                <span>User : <strong>{user.username}</strong> </span><br/>
