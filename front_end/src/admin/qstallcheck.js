...
const Qstallcheck = () => {
  ...
  const GetListsQuest = (e) => {
    try{
      e.preventDefault();
      apiAdmin
        .get(`${"https://stawro.xyz"}/get/admin/all/question/lists/${lang}`)
        .then((res) => {
          if (res.data.data) {
  ...
  const Delet = () =>{
    try{
      axios.delete(`https://stawro.xyz/delete/unwanted/questions/${id}`)
      .then(res =>{
      ...
      <center>
        ...
        <div className="admin_qstallcheck-main-cnt-01">
          {qdata.map((user, i) => {
            ...
            const GetQustNo = (qno) =>{
              try{
                apiAdmin.get(`${"https://stawro.xyz"}/question/one/by/${qno}/${lang}`)
                .then(res => {
        ...
        <div className="admin_qstallcheck-remaining-cnt">
          ...
          {remaining_data.length > 0 ? 
            remaining_data.map((num, i) =>{
              ...
              const GetQustNo = (num) =>{
                try{
                  apiAdmin.get(`${"https://stawro.xyz"}/question/one/by/${num}/${lang}`)
                  .then(res => {
                    if(res.data.question){
                      setShow1(true)
                      setQstData(res.data.question)
