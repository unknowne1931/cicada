...
const QuestionAdd = () => {
    ...
    const GetAllDAta = () =>{
        try{
            apiAdmin.get("https://stawro.xyz/get/all/admin/new/languages/data")
            .then(res =>{
                if(res.data.Data){
    ...
    const PostData = (e) =>{
        try{
            setAlert(false);
            e.preventDefault();
            axios.post(`${"https://stawro.xyz"}/get/posted/count/questions`,{user, img, Questio, a, b, c, d, Ans, lang, tough, seconds})
            .then(res =>{
    ...
    const PostAllLang = (e) => {
        try{
            e.preventDefault();
            setAlert(false);
            apiAdmin.post("https://stawro.xyz/add/all/admin/new/languages/data",{lang : ALL})
            .then(res =>{
                if(res.data.Status === "OK"){
                    GetAllDAta()
                    setShow3(false)
            ...
            <div>
                {ALLLData.map((users, i) =>{
                    ...
                    const DeleOne = () =>{
                        try{
                            setAlert(false)
                            apiAdmin.delete(`https://stawro.xyz/delete/all/selected/data/with/onley/one/${users}`)
                            .then(res =>{
                                if(res.data.Status === "OK"){
                                    GetAllDAta()
